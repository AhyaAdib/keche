...
public class SampleWebView : MonoBehaviour
{
    ...
    IEnumerator DeleteFirst()
    {
        ...
        var go = GameObject.Find("WebViewObject");
        Destroy(go);
    }
    IEnumerator Starting()
    {
        // GameObject g = GameObject.Find("WebViewObject");
        ...
        webViewObject.Init(
            ...
            //         body {
            //             border-radius: 20px;
            //             border: 10px solid #BF6D33; /* Customize the border as needed */
            //         }
            //     `;
            //     document.head.appendChild(style);
            // ";

            // webViewObject.EvaluateJS(css);
        ...
        webViewObject.SetMargins(320, 200, 320, 120);
...
/*
    void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(x, 180, 80, 80), "<=")) {
        ...
        if (GUI.Button(new Rect(x, 180, 80, 80), "Reload")) {
            webViewObject?.Reload();
        }
        x += 90;
        GUI.TextField(new Rect(x, 180, 180, 80), "" + ((webViewObject == null) ? 0 : webViewObject.Progress()));
        x += 190;
        ...
        if (GUI.Button(new Rect(x, 180, 80, 80), "c")) {
            webViewObject?.GetCookies(Url);
        }
        x += 90;

        if (GUI.Button(new Rect(x, 180, 80, 80), "x")) {
            webViewObject?.ClearCookies();
        }
        x += 90;

        if (GUI.Button(new Rect(x, 180, 80, 80), "D")) {
            webViewObject?.SetInteractionEnabled(false);
        }
        x += 90;

        if (GUI.Button(new Rect(x, 180, 80, 80), "E")) {
    ...
    public void ControlWebView(int action)
    {
        var g = GameObject.Find("WebViewObject");
        switch (action)
        {
            ...
            case 3:
                if (g != null) {
                    Destroy(g);
                    g = GameObject.Find("WebViewObject");
                    if (g != null) 
                        Destroy(g); 
                } 
                break;
            ...
            // ... other actions, you can add cases for other functionalities.
            // For example, destroying, enabling/disabling interaction, clearing cookies, etc.
        }
    }
}
